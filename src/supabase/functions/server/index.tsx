...
app.post("/make-server-fc862019/gallery/sync-from-github", async (c) => {
  try {
    ...
    // Store a proxy-relative marker the frontend resolves to the full proxy URL.
    // Format: "gallery-img:{category}/{slug}/{file}" — frontend converts this
    // to ${serverUrl}/gallery/img/{category}/{slug}/{file} at render time.
    // Using a marker avoids hard-coding the Supabase project URL in KV records.
    const rawUrl = (fullPath: string) => {
      const rel = fullPath.replace(/^(?:public\/)?gallery\//, '');
      return `gallery-img:${rel}`;
    };
...
// ─── Image proxy ────────────────────────────────────────────────────────────
// Images live in the private GitHub repo at gallery/{category}/{slug}/{file}.
// Vercel only serves public/ — root-level gallery/ is never deployed.
// This proxy fetches the image (authenticated) and streams it to the browser.
// The browser caches aggressively (1 day) so repeated loads are nearly free.
app.get("/make-server-fc862019/gallery/img/*", async (c) => {
  try {
    const GITHUB_TOKEN = Deno.env.get('GITHUB_TOKEN');
    ...
    // imagePath = everything after /gallery/img/ e.g. "Body/pt_10/file.png"
    const imagePath = c.req.param('*');
    if (!imagePath) return c.json({ error: 'No image path' }, 400);

    // Try both repo roots — images may be at gallery/ or public/gallery/
    for (const root of ['gallery', 'public/gallery']) {
      const rawUrl = `https://raw.githubusercontent.com/${GITHUB_OWNER}/${GITHUB_REPO}/main/${root}/${imagePath}`;
    ...
    console.warn(`[Gallery Img Proxy] 404 for path: ${imagePath}`);
    return c.json({ error: 'Image not found' }, 404);
  } catch (err) {
    console.error('[Gallery Img Proxy] Error:', err);
